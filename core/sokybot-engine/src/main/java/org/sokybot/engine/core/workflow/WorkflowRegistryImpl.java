    ...
    public int registerCycle(ICycleDefinition cycle) {
        ...
        try {
            ...
            if (cycles.containsKey(cycleName)) {
                log.info("Cycle '{}' already registered, replacing", cycleName);
                // Unregister existing cycle to allow re-registration
                lock.writeLock().unlock();
                try {
                    unregisterCycle(cycleName);
                } finally {
                    lock.writeLock().lock();
                }
            }
    ...
    private void validateCycle(ICycleDefinition cycle) {
        ...
        if (!entryStateFound) {
            throw new IllegalArgumentException(
                    "Entry state '" + entryStateName + "' not found in cycle '" + cycle.getName() + "'");
        }
