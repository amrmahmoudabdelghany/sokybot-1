...
import org.sokybot.runtime.INetworkController;
...
public class ConnectionService implements EventHandler {
    ...
    public ConnectionService(
        ...
        this.settingsProvider = settingsRegistry.getProvider(
                gameStateProvider.getGroupName(),
                gameStateProvider.getMachineName(),
                "login",
                LoginSettings.class);
            ...
            switch (action) {
                ...
                case "saveProfile":
                    ...
                    if (saveName != null && !saveName.isEmpty()) {
                        profileManager.saveProfile(gameStateProvider.getGroupName(), saveName);
                    }
                    break;
                ...
                case "loadProfile":
                    ...
                    if (loadName != null && !loadName.isEmpty()) {
                        profileManager.loadProfile(gameStateProvider.getGroupName(), gameStateProvider.getMachineName(),
                                loadName);
                ...
                case "deleteProfile":
                    ...
                    if (delName != null && !delName.isEmpty()) {
                        profileManager.deleteProfile(gameStateProvider.getGroupName(), delName);
                    }
                    break;
    ...
    private void updateSettings(Map<String, Object> data) {
        settingsProvider.update(settings -> {
            if (data.containsKey("username"))
                settings.setUsername((String) data.get("username"));
            if (data.containsKey("password"))
                settings.setPassword((String) data.get("password"));
            if (data.containsKey("targetGateway"))
                settings.setTargetGateway((String) data.get("targetGateway"));
            if (data.containsKey("autoLogin"))
                settings.setAutoLogin((Boolean) data.get("autoLogin"));
            if (data.containsKey("targetAgent"))
                settings.setTargetAgent((String) data.get("targetAgent"));
        });
    }
    ...
    private Map<String, Object> getState() {
        ...
        // Hide password if locked (though settings provider might return encrypted
        // string)
        if (!unlocked) {
        ...
        // Connection state
        boolean connected = networkController.getProxyConnection().isConnected();
        data.put("connected", connected);

        // Profiles
        data.put("profiles", profileManager.listProfiles(gameStateProvider.getGroupName()));
