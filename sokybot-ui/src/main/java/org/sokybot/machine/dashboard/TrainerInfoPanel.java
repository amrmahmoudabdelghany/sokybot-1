...
public class TrainerInfoPanel extends JPanel implements ActionListener, PropertyChangeListener {
	...
	private Image processImage(Image image, int w, int h, int cornerRadius) {
		...
		case Trainer.ENTITY_PROPERTY:
			...
			this.gameDao.findCharacterIcon(this.trainer.getRefId()).ifPresent((image)->{
				...
				this.lblCharIcon.setIcon(
						new ImageIcon(processImage(image, CHAR_IMAGE_WIDTH, CHAR_IMAGE_HEIGHT, CHAR_IMAGE_CRADIUS)));
			});
			break;
