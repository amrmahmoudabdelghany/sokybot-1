...
public class SkillEntityExtractor implements IExtractor {
	...
	@Reference
	public SkillEntityExtractor(CacheManager cacheManager , SkillEntityRepository repo) {
		this.cache = cacheManager.getCache(CACHE_NAME) ; 
		this.skillEntityRepository = repo ; 
	...
	public void extract(IPk2Driver driver) {
	  ...
	  this.skillEntityRepository.saveAll(skills) ; 
				
				//.forEach((skill) -> {
				///	this.cache.put(skill.getLongId(), skill.getRefId()) ; 					
				//	this.skillEntityRepository.save(skill) ; 
				//});
