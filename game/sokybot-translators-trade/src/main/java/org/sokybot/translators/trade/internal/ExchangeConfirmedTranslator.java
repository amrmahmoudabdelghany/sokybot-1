...
enum ExchangeConfirmedTranslator implements IPacketTranslator {
    INSTANCE;

    @Override
    public int getOpcode() {
        return TradePacketOpcodes.EXCHANGE_CONFIRMED;
    }
    ...
    public List<IGameEvent> translate(String machineFullName, ImmutablePacket packet, ChunkedPacketManager chunkManager) {
        try {
            ...
            boolean self = sr.getByte() != 0;
            return Collections.singletonList(new TradePartnerConfirmed(machineFullName, exchangeId, self));
