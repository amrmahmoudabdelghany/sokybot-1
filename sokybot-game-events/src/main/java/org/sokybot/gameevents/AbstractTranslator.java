 ...
 * Instances are created per-game (shared across all bots) for memory optimization.
 * 
 * <p>Translators must be thread-safe since they're shared across bots.
 * Per-bot state (like ChunkedPacketManager) is accessed via ChunkedPacketManagerRegistry.
 */
public abstract class AbstractTranslator implements IPacketTranslator {
     ...
     * Subclasses implement this method. Chunk manager can be accessed via getChunkManager().
     * 
     * @param machineFullName The machine identifier
     * @param packet The packet to translate
     * @return List of events
     ...
     * 
     * <p>Translators that need chunk manager (e.g., CharacterDataBeginTranslator,
