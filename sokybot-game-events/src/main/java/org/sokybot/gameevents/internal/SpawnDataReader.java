...
import org.sokybot.game.enums.InteractionMode;
...
public class SpawnDataReader {
    ...
    private void readSharedSpawnData() {
        ...
        shared.angle = (short) SilkroadUtils.getAngle(reader.getShort()); // Angle in degrees? SilkroadUtils converts it.
        
        float worldX = SilkroadUtils.getXCoord(shared.xOffset, (short)shared.xSector, 10); // Check multiplier 10 or 100? SpawnParser used 10 for Location but 100 in Event? 
        ...
        // Wait, getting world coordinates usually is standard. I'll use SilkroadUtils defaults if possible or stick to 10.
        
        float worldY = SilkroadUtils.getYCoord(shared.yOffset,(short) shared.ySector, 10);
        
        shared.position = new Position(worldX, worldY, shared.zOffset); 
    }
    ...
    private void readFighterData() {
        ...
        // Skipping assignment to DTO for now as MonsterData doesn't have Movement fields yet
        ...
        for(int i=0; i<buffCount; i++) {
            ...
            lookup.findSkill(skillId).ifPresent(skill -> {
                 ...
                 // HOWEVER, `Skill` object in `api` had `isTransferableBuff`. `SkillEntity` in persistence might not?
    ...
    public MonsterData readMonster(NPCEntity entity) {
        ...
        switch(strengthLevel) {
            case 0: type = MonsterType.Normal; break;
            case 1: type = MonsterType.Champion; break;
            case 4: type = MonsterType.Giant; break;
            // Add other cases if known
        }
         // Assumed conversion
         
        int maxHp = entity.getHP();
        if(type == MonsterType.Champion) maxHp *= 2;
        else if(type == MonsterType.Giant) maxHp *= 5; // Simplified
    ...
    public PlayerData readPlayer(NPCEntity entity) {
        ...
        reader.getByte(); reader.getByte(); // unk
        ...
        // and handle Player later if needed, or if I must advanced stream I must implement it.
        // Since EntitySpawnTranslator uses this, if I don't implement it, Player spawns will fail/crash.
        // But EntitySpawnTranslator logic branches on Type. If I don't update EntitySpawnTranslator to call this for Player, it won't crash.
        // I will ONLY call readMonster and readDropItem in EntitySpawnTranslator for now.
        return null; 
    }
}
