...
public class ShopEntityExtractor implements IExtractor {
	...
	@Autowired
	public ShopEntityExtractor(CacheManager cacheManger , ItemEntityRepo itemRepo , ShopEntityRepo shopRepo , NPCEntityRepo npcEntityRepo) { 
		this.cache = cacheManger.getCache(CACHE_NAME) ; 
		this.itemEntityRepo = itemRepo ; 
		this.shopEntityRepo = shopRepo ; 
		this.npcEntityRepo = npcEntityRepo ; 
	...
	public void extract(IPk2Driver driver) {
		...
		List<ShopEntity> res = 	Pk2ExtractorUtils.toCSVRecordStream(jmx).map((record) -> {
				...
				NPCEntity entity = this.npcEntityRepo
						.findById(shopNpcId)
						.orElseGet(()->{
				...
				if(entity.getLongId().equals("UNKNOWN"+shopNpcId)) { 
					...
					entity = this.npcEntityRepo.save(entity) ; 
		  ...
		  ;
		this.shopEntityRepo.saveAll(res) ; 
		});
	...
	private void extractPackageItems(IPk2Driver driver) {
		driver.findFirst("refscrapofpackageitem.txt").ifPresent((jmx) -> {
			Pk2ExtractorUtils.toCSVRecordStream(jmx).filter((r) -> r.size() > 3).forEach((r) -> {
				...
				ItemEntity itemEntity = this.itemEntityRepo.findItemEntityByLongId(itemLongId)
